...
const createNewSession = async (req, res) => {
	try {
		...
		if (!checkDiagnostic || !checkChild || !checkUser[0]) {
			let errorMessage = '';
			if (!checkDiagnostic) {
				errorMessage = 'There is no diagnostic with this id';
			} else if (!checkChild[0]) {
				errorMessage = 'There is no child with this id';
			} else {
...
const getDiagnosticForEvaluation = async (req, res) => {
	try {
		const content = await diagnosticService.getDiagnosticContentForEvaluation(req.params.id, req.query.session);
		return res.status(200).json({
			status: 200,
			data: content,
			message: 'retrieved diagnostic content successfully'
