...
const getDiagnosticResultAnalysisAdditional = async (req, res) => {
	try {
		const content = await evaluationService.getDiagnosticResultAnalysisAdditional(req.query.session);
		return res.status(200).json({
			status: 200,
			data: content,
			message: 'retrieved additionnal analysis successfully'
		});
	} catch (error) {
		return res.status(500).json({
			status: 500,
...
const setDiagnosticResultAnalysisAdditional = async (req, res) => {
	try {
		const content = await evaluationService.setDiagnosticResultAnalysisAdditional(req.body);
		return res.status(200).json({
			status: 200,
			data: content,
			message: 'updated additionnal analysis successfully'
		});
	} catch (error) {
		return res.status(500).json({
			status: 500,
