...
function EvaluationItemComponent({ score, t, handleClickTab, session }) {
	...
	function customEncodeSpecialCharss(url) {
		const specialChars = {
			' ': '%20',
			ü: '%C3%BC',
			ä: '%C3%A4',
			...
			ö: '%C3%B6',
			ß: '%C3%9F',
			Ü: '%C3%9C' // Add the encoding for Ü
			// Add more special characters as needed
		};

		const normalizedUrl = url.normalize('NFC'); // Normalize the string
		const encodedUrl = normalizedUrl.replace(/[\süäÄÖÜößÜ]/g, match => specialChars[match]);
		return process.env.REACT_APP_S3_ENDPOINT + '/files/downloads/all/' + encodedUrl;
	}
					...
					<div className={'callout ' + (score.class !== undefined ? score.class : 'warning')}>
						...
						{score?.link?.label && (
							<p>
								<a
									className="button"
									href={customEncodeSpecialCharss(score?.link.href)}
									target="_blank"
