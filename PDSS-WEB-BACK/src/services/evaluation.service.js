...
const { evaluationsQueries } = require('../../config/query');
			...
			result.scores.map(async score => {
				...
				switch (score.type) {
					...
					case 'table':
						...
						break;
					case 'message':
						if (score.scoreName === 'Phonetik') {
							data = { label: score.label, append_text: score.append_text };
						}
						else data = { label: score.label, link: score.link };
						break;
					case 'compact_values':
					case 'questions':
					case 'answers':
...
const getAnalysesResultScores = async body => {
	...
	if (diagnosticId == 9) {
		rawValueAkkusativ = await getRawValueByTagName(body.session, 'Akkusativ');
		rawValueDativ = await getRawValueByTagName(body.session, 'Dativ');
	}
	const props = {
		result,
		diagnosticId,
	...
	if (childLanguage === 'm') {
		let bilingualLink = '';
		switch (true) {
			case childAgeInMonths < 36:
				bilingualLink = "Orientierungswerte für mehrsprachige Kinder im Alter von 2.pdf";
				break;

			case childAgeInMonths < 48:
				bilingualLink = "Orientierungswerte für mehrsprachige Kinder im Alter von 3.pdf";
				break;

			case childAgeInMonths < 60:
				bilingualLink = "Orientierungswerte für mehrsprachige Kinder im Alter von 4.pdf";
				break;

			case childAgeInMonths < 72:
				bilingualLink = "Orientierungswerte für mehrsprachige Kinder im Alter von 5.pdf";
				break;

			default:
				bilingualLink = "Orientierungswerte für mehrsprachige Kinder im Alter von 6.pdf";
		}
		scores.unshift({
			scoreName: 'Bitte beachten',
			type: 'message',
			visible: 'yes',
			label: 'label_bilingual_child',
			link: {
				label: 'btn_show_orientation_values',
				href: bilingualLink,
			}
		});
	}
...
const getAnalysesValue = async props => {
	const { result, diagnosticId, rawValue, body, rawValueAkkusativ, rawValueDativ, childAgeInMonths, totalValue } =
		props;
	let scoreTable = [],
		scores = [];
	let total = 0,
	...
	const scorePromises = result?.[0]?.map(async item => {
		...
		switch (diagnosticId) {
			...
			case 5:
				...
				switch (score_name) {
					case 'MLU':
						const resultMLU = await getRawValueForExtendAnswer(body.session, score_name);
						raw_value_specific = resultMLU.rawValue;
						decimals = resultMLU.decimals;
						break;
					case 'Vollständigkeit':
						const resultVoll = await getRawValueForExtendAnswer(body.session, score_name);
						raw_value_specific = resultVoll.rawValue;
						ratio = resultVoll.ratio;
						decimals = resultVoll.decimals;
						break;
					case 'Score A':
...
const getScoreForTest9 = async options => {
	...
	await Promise.all(
		resultExtended?.[0]?.map(async item => {
			...
			scores[foundIndex].values.push({
				name: item.question_id,
				score: resultScoreAkkusativ[0]?.[0].t_value || 0
			});
			totalAkkusative += resultScoreAkkusativ[0]?.[0].t_value || 0;
			...
			scores[foundIndexD].values.push({
				name: item.question_id,
				score: resultScoreDativ[0]?.[0].t_value || 0
			});
			totalDativ += resultScoreDativ[0]?.[0].t_value || 0;
	...
	totalDativ += dativ_used;
	totalAkkusative += akkusativ_used;
...
const findIndexByScoreName = (scores, scoreName) => {
	return scores[scores.findIndex(x => x.scoreName == scoreName)]
}
	...
	try {
		let scorePromises = scores.map(async item => {
			...
			let value = {
				...
				decimals: item.scoreName === 'MLU' ? 1 : 0,
				width: 'small-3'
			};
			return value;
		});
		...
		scoresExtended.push(findIndexByScoreName(scores, 'MLU'),
			findIndexByScoreName(scores, 'Vollständigkeit'),
			findIndexByScoreName(scores, 'Score A'),
			findIndexByScoreName(scores, 'Score B'));

		findIndexByScoreName(scoresExtended, 'Situationsbilder beschreiben').values.push({
		...
		let parsedQuestions = fragen[0][0] ? JSON.parse(fragen[0][0].data) : null;
		if (parsedQuestions && parsedQuestions.values) {
			parsedQuestions.values.forEach(item => {
				if (item.answer && item.answer == 'correct') needs_grammar_analysis = true;
				findIndexByScoreName(scoresExtended, 'Fragen').values.push(item);
			});
		} else {
		...
		resultExtend?.[0]?.forEach(item => {
			item.additional = JSON.parse('[' + item.additional + ']');
			findIndexByScoreName(scoresExtended, 'Antworten').values.push(item);
		});
...
const calculateScores = data => {
	...
	let group_a_score = 0;
	let group_b_score = 0;
	let group_a_score_total = 0;
	let group_b_score_total = 0;
	...
	data?.[0].forEach(item => {
		...
		item.has_score = selected >= item.min_occur;

		if (item.belongs_to === 0) {
			group_a_score_total += item.score;
			...
			group_b_score_total += item.score;
...
const countOccurrences = (html, regex) => (html.match(regex) || []).length;

const calculateStructureCount = (phonetic_contents_data, totalValue) => {
	let count_initial_consonant_removed = 0;
	let count_initial_consonant_total = 0;
	let count_initial_bonding_removed = 0;
	let count_initial_bonding_total = 0;
	let count_final_consonant_removed = 0;
	let count_final_consonant_total = 0;
	let count_final_bonding_removed = 0;
	let count_final_bonding_total = 0;
	let count_bonding_reduction = 0;
	let count_bonding_reduction_total = 0;
	let count_deaffrication = 0;
	let count_deaffrication_total = 0;
	let count_addition = 0;

	phonetic_contents_data.forEach(item => {
		let html = item.answer_04 !== null ? item.answer_04 : item.target_item_html;

		count_initial_consonant_removed += item.json?.structures?.initial_consonant_removed || 0;
		count_initial_bonding_removed += item.json?.structures?.initial_bonding_removed || 0;
		count_final_consonant_removed += item.json?.structures?.final_consonant_removed || 0;
		count_final_bonding_removed += item.json?.structures?.final_bonding_removed || 0;
		count_bonding_reduction += item.json?.structures?.bonding_reduction || 0;
		count_deaffrication += item.json?.structures?.deaffrication || 0;

		count_initial_consonant_total += countOccurrences(html, /letter initial/g);
		count_initial_bonding_total += countOccurrences(html, /bond initial/g);
		count_final_consonant_total += countOccurrences(html, /letter final/g);
		count_final_bonding_total += countOccurrences(html, /bond final/g);
		count_bonding_reduction_total += countOccurrences(html, /bond/g);
		count_deaffrication_total += countOccurrences(html, /affricate/g);
		count_addition += countOccurrences(html, /added/g);
	});
	...
	return [
		...
		{ name: 'label_final_bonding_removed', value: `${count_final_bonding_removed}/${count_final_bonding_total}` },
		{ name: 'label_bonding_reduction', value: `${count_bonding_reduction}/${count_bonding_reduction_total}` },
		{ name: 'label_deaffrication', value: `${count_deaffrication}/${count_deaffrication_total}` },
		{ name: 'label_addition', value: `${count_addition}/${totalValue}` }
	];
};

const getSyllablesStructure = async (phonetic_contents_data, totalValue) => {
	return calculateStructureCount(phonetic_contents_data, totalValue);
};
...
const getDiagnosticResultAnalysisAdditional = async (session) => {
	try {
		let response = await execute(SQL.evaluationsQueries.getResultAnalysisAdditional(session));
		return response[0];
	} catch (error) {
		console.error(error);
		throw error;
...
const setDiagnosticResultAnalysisAdditional = async body => {
	let results;
	const { session, value, name } = body;
	if (session) {
		let result = await execute(SQL.evaluationsQueries.getResultAnalysisAdditional(session));

		if (name && value) {
			if (result[0].length > 0) {
				results = await execute(SQL.evaluationsQueries.updateResultAnalysisAdditional(session, name, value));
			} else {
				results = await execute(SQL.evaluationsQueries.setResultAnalysisAdditional(session, name, value));
			}
		}
	}
	return results;
...
module.exports = {
	...
	updateQuestionTest5,
	getDiagnosticResultAnalysisAdditional,
	setDiagnosticResultAnalysisAdditional
};
