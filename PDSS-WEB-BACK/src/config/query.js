...
module.exports = {
	...
	diagnosticsQueries: {
		...
		diagnosticSessions: {
			getDiagnosticSession: userId =>
				`SELECT diagnostic_session.*,(
					SELECT TRUNCATE(
							COUNT(answer) * 100.0 / (
								select COUNT(*)
								from diagnostic_content
								where diagnostic_content.diagnostic = diagnostic_session.diagnostic
									AND (
										CASE
											WHEN diagnostic_session.started = "training" THEN diagnostic_content.training = "yes"
											ELSE diagnostic_content.training = "no"
										END
									)
									AND age_min <= diagnostic_session.child_age_in_months
									AND age_max >= diagnostic_session.child_age_in_months
							),
							0
						)
					FROM diagnostic_result
					where diagnostic_result.session = diagnostic_session.session
						and diagnostic_result.diagnostic_content in (
							select id
							from diagnostic_content
							where (
									CASE
				...
				) as process_percent, diagnostic.title, (SELECT use_in_profile FROM diagnostic_result_analysis WHERE session = diagnostic_session.session and type='values' LIMIT 1) as use_in_profile 
		...
		DiagnosticResultQueries: (session, contentId, body, methodName) => {
			let setters = '',
				answer = '',
				updatesSettlers = '';
			if (methodName == 'POST') {
    ...
    WHERE session='${session}' AND diagnostic_content_grammar='${diagnostic_content_grammar_id}' AND diagnostic_result_extended_answer='${diagnostic_result_extended_answer}'`,
		...
		updateDiagnosisResultScores: (session, score_name, tvalue, data) =>
			`UPDATE diagnostic_result_analysis SET tvalue='${tvalue}' ,data='${JSON.stringify(data)}' WHERE score_name='${score_name}' AND session='${session}'`,
	...
	diagnostic_content.age_max>=${childAgeInMonths} AND diagnostic_content.diagnostic=2 AND diagnostic_content.training='no' HAVING phonetic_structure IS NOT NULL`,
		...
		getResultAnalysisAdditional: session => {
			return `SELECT * FROM diagnostic_result_analysis_additional WHERE session='${session}'`
		},
		setResultAnalysisAdditional: (session, name, value) => {
			return `INSERT INTO diagnostic_result_analysis_additional (session,name,value) VALUES ('${session}', '${name}','${value}')`
		},
		updateResultAnalysisAdditional: (session, name, value) => {
			return `UPDATE diagnostic_result_analysis_additional SET value='${value}' WHERE session='${session}' AND name='${name}'`
		}
	},
	recordQueries: {
