...
module.exports = {
		...
		try {
				...
				if (body.result) {
					...
					let check = await execute(
						SQL.diagnosticsQueries
							.DiagnosticResultQueries(body.session, content, body.result, "CHECK")
					...
					if (check[0].length > 0) {
						data = await execute(
							SQL.diagnosticsQueries.DiagnosticResultQueries(body.session, content, body.result, "UPDATE").update()
						);
						const sessionDetails = await execute(SQL.diagnosticsQueries.getDiagnosisSessionDetails(body.session));
						const sessionStatus = sessionDetails?.[0]?.[0]?.status;
						if (sessionStatus && sessionStatus == 'finished') {
							await this.updateDiagnosisResultAnalyses(body.session);
						}
					}
					// Otherwise, set a new result
						...
						data = await execute(
							SQL.diagnosticsQueries
								.DiagnosticResultQueries(body.session, content, body.result, "POST")
								.setNewOne()
						);
					}
				}
			...
			// Otherwise, update or set a new diagnostic result
				...
				// Otherwise, set a new result
					...
					console.log(SQL.diagnosticsQueries.DiagnosticResultQueries(body.session, content, body.result, "POST").setNewOne())
					data = await execute(
						SQL.diagnosticsQueries.DiagnosticResultQueries(body.session, content, body.result, "POST").setNewOne()
					);
				}
	...
	updateDiagnosisResultAnalyses: async function (session) {
		try {
			if (session) {
				const body = { session: session };
				const { scores } = await getAnalysesResultScores(body);
				for (const score of scores) {
					let data = {};
					switch (score.type) {
						case 'values':
					...
					const tvalue = score.tvalue || 0;
					const sql = SQL.diagnosticsQueries.updateDiagnosisResultScores(
						session,
						score.scoreName,
						tvalue,
						data
